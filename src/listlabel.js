...
/*
                        var frame = function () {
                            var tmp   = element.scrollTop,
                                delta = speed * (Date.now() - tLast);

                            tmp += delta;
                            tLast = Date.now();

                            if (tmp <= element.scrollHeight - element.clientHeight)
                                refresh(tmp);
                            element.scrollTop = tmp;
...
/*                            speed = 0;
                        ...
                        element.ontouchmove = function (event) {
                            ...
                            var deltaY = yStart - event.pageY + sStart;

                            if (deltaY <= element.scrollHeight - element.clientHeight)
                                refresh(deltaY);
                            element.scrollTop = deltaY;
/*
                            speed = speed * 0.05 + (event.pageY - yLast) / (Date.now() - tLast) * 0.15;
                            tLast = Date.now();
                            yLast = event.pageY; */
                        };
/*
